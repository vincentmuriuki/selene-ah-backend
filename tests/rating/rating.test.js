...
describe('Rating controller', () => {
  ...
  describe(`#POST ${ratingUrl} - Endpoint to add a rating -`,() => {
    it('should rate an article', async () => {
      ...
      .post(`${ratingUrl}`)
      .send({'rating': ratingObject.rating})
    ...
    async () => {
      const response = await chai.request(app)
        ...
        .send({ articleId: '273da52b-b46f-48d3-b180-4eb31eec638b', rating: 2 })
        .set('Authorization', readerToken);
...
*/
  describe(`#PUT ${ratingUrl} - Endpoint to update a rating -`,() => {
    ...
    it('should return a 500 error if the update action fails.', async () =>{
      ...
      await RatingController.updateRatingById(req, res, next);
      expect(next.called).to.be.true;
      sinon.restore();
    });
  });
  ...
  async () => {
    ...
    async () =>{
      ...
      await RatingController.getSingleAticleRatings(req, res, next);
      expect(next.called).to.be.true;
      sinon.restore();
    });
  });
